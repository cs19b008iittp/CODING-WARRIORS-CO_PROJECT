...
int mem[1024]={0};
int R[32]={0};
float total_access=0,l1_misses=0,l2_misses=0,tot_misses=0;
float l1_missrate, l2_missrate;
int c1,cache_size1,a1,blocksize1,cache_size2, a2, blocksize2, c2, m1,h=0,q=0;
int num_blocks_l1, num_blocks_l2;
int sets1=0, sets2=0;
int* tagl1[1000][1000]={NULL};
int* tagl2[1000][1000]={NULL};
int cachel1[1000][1000]={0};
int cachel2[1000][1000]={0};
int i,counter1=0;
int n=0;
int pin=0;
int clock_1=0;
      ...
      test = (int*)((uintptr_t)test % sets1); 
        ...
        if(test==trail)
        {
            ...
            for(int i=0;i<a1;i++)
            {
             ...
             tagl1[h][pqr1[h]%a1]=ptr;
             cachel1[h][pqr1[h]%a1]=data;
             return; 
        }
              trail++;
        }
   ...
   int l1;
    ...
    catch (const std::out_of_range& oor)
    {
     ...
     stringstream geek1(str3); 
       ...
       int temp=R[l1];   
       //lw R13 R9
      int s1=  search_cachel1(&mem[temp]);
      if(s1!=-2)
      {
           R[l]=s1;
           ckl=ckl+c1;
      ...
      else if(( search_cachel2(&mem[temp]))!=-2)
      {
         l1_misses++;
           R[l]= search_cachel2(&mem[temp]);
           ckl=ckl+c1+c2;
           return;
      }
      else
      {
          ...
          l2_misses++;
           R[l]=mem[temp];
           addl1(&mem[temp],mem[temp]);
           addl2(&mem[temp],mem[temp]);
           ckl=ckl+c1+c2+m1;
           return;
      }
...
void storeword(string str, string str1,int x)
{
    ...
    catch (const std::out_of_range& oor)
    {
     ...
     stringstream GEEK(STR1); 
       ...
       arr[x][2]=32;
       mem[R[l2]] = R[l];   
       if(search_cachel1(&mem[R[l2]])!=-2)
       {
            store_cachel1(&mem[R[l2]], R[l]);
       }
        if(search_cachel2(&mem[R[l2]])!=-2)
       {
            store_cachel2(&mem[R[l2]], R[l]);
       }
}
void jump(int l);
...
void branch_ins();
  void jump(int l)
  {
     ...
     cout<<"\n----------------------------------------------------------------------------------------------------------------------------\n";
        ...
        cout<<"-----------------------------------------------------------------------------------------------------------------------------\n";
        cout<<"number of stalls: ";
        cout<<(var +count1-1+ckl)<<endl;
        }
         cout<<"number of misses in cache l1: "<<l1_misses<<endl;
         cout<<"number of misses in cache l2: "<<l2_misses<<endl;
         cout<<"total number of accesses :"<<total_access<<endl;
         cout<<"total number of misses in cache l1 and l2: "<<l1_misses+l2_misses<<endl;
         l1_missrate=l1_misses/total_access;
         l2_missrate=l2_misses/total_access;
         float tot_missrate=(l1_misses+l2_misses)/total_access;
         cout<<"l1 cache miss rate: "<<l1_missrate<<endl;
         cout<<"l2 cache miss rate: "<<l2_missrate<<endl;
         cout<<"total miss rate: "<<tot_missrate<<endl;
          exit(0);
      }
      else
      {
   ...
   void pipeline_withdataforwarding()
   {
        ...
        else
        {
             ...
             clock_2=clock_2+x+cnt_branch-1+ckl;
   ...
   void pipeline_withstalls()
   {
       ...
       else
       {
           ...
           clock_2= clock_2+ x+cnt_branch+count1-1+ckl;
           cout<<"Total number of cycles : "<<clock_2<<endl;
      }
      float ipc= (float)x/clock_2;
     cout<<"IPC "<<ipc<<endl;  
...
int main () {
   ...
   if (!infile) {
    ...
    else
    {
         int j=0;
    string str;
    cout<<"enter cache size of l1 cache: "<<endl;
    cin>>cache_size1;
    cout<<"enter block size of l1 cache :"<<endl;
    cin>>blocksize1;
    cout<<"enter associativity of l1 cache :"<<endl;
    cin>>a1;
    cout<<"enter access latency of l1 cache: "<<endl;
    cin>>c1;
     cout<<"enter cache size of l2 cache: "<<endl;
    cin>>cache_size2;
    cout<<"enter block size of l2 cache :"<<endl;
    cin>>blocksize2;
    cout<<"enter associativity of l2 cache :"<<endl;
    cin>>a2;
    cout<<"enter access latency of l2 cache: "<<endl;
    cin>>c2;
    cout<<"enter the memeory access time :"<<endl;
    cin>>m1;
    num_blocks_l1= cache_size1/blocksize1;
    num_blocks_l2=cache_size2/blocksize2;
    
    sets1=num_blocks_l1/a1;
    sets2=num_blocks_l2/a2;
